...
for dinosaurio in inventario_dinosaurios:
    if (
        dinosaurio[1] == "Velociraptor"
    ):  # Comprueba si la especie (índice 1) es "Velociraptor"
        velociraptors_list.append(
            dinosaurio[0]
        )  # Añade el nombre (índice 0) a la lista
...
for dinosaurio in inventario_dinosaurios:
    if (
        dinosaurio[3] == "Necesita Atención"
    ):  # Comprueba si el estado de salud (índice 3) es "Necesita Atención"
        dinosaurios_necesitan_atencion += 1  # Incrementa el contador
print(f"\n--- Dinosaurios que necesitan atención médica ---")
print(
    f"Número de dinosaurios que necesitan atención médica: {dinosaurios_necesitan_atencion}"
)
...
def inspeccionar_recinto(recinto_numero):
    ...
    for dinosaurio in inventario_dinosaurios:
        if (
            dinosaurio[2] == recinto_numero
        ):  # Comprueba si el recinto (índice 2) coincide con el recinto_numero dado
            print(f"- Nombre: {dinosaurio[0]}, Estado: {dinosaurio[3]}")


inspeccionar_recinto(2)  # Llama a la función para inspeccionar el Recinto 2
